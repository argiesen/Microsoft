...
function Install-SQLInstance {
	...
	if (Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\Instance Names\SQL" -Name "$Instance" -ErrorAction SilentlyContinue){
		Write-Log "SQL Instance ($Instance) already installed" -Indent $Indent -OutTo $LogOutTo
		return
	}
...
Install-CsSQLInstance -Instance $SQLInstance -SQLMediaDir $SQLMediaDir -SQLPath $SQLPath -OpenPorts | Out-Null